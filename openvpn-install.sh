...
function buildOpenVPNOTP() {
	git clone https://github.com/evgeny-gridasov/openvpn-otp.git /tmp/openvpn-otp
	cd /tmp/openvpn-otp
	./autogen.sh
...
function installOpenVPN() {
	...
	if [[ ! -e /etc/openvpn/server.conf ]]; then
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			# openvpn-otp dependency
			apt-get install -y git autoconf automake libtool libssl-dev libpam-google-authenticator qrencode
			# build openvpn-otp
			buildOpenVPNOTP
		elif [[ $OS == 'centos' ]]; then
...
plugin \"/usr/lib/openvpn/openvpn-otp.so\" \"otp_slop=30\"
reneg-sec 86400" >>/etc/openvpn/server.conf
...
function newClient() {
	...
	# Generate secret
	google-authenticator -t -d -f -r 3 -R 30 -W -C -s "/etc/openvpn/google-auth/$CLIENT" || { echo -e "Error generating QR code"; exit 1; }
	PASSWORD=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 12)
	SECRET=$(head -n 1 "/etc/openvpn/google-auth/$CLIENT")
	PUBLICIP=$(curl -s https://api.ipify.org)
	qrencode -t PNG -o "$homeDir/$CLIENT.png" "otpauth://totp/$CLIENT@$PUBLICIP?secret=$SECRET&issuer=openvpn" || { echo -e "Error generating PNG"; exit 1; }
	echo "$CLIENT otp totp:sha1:base32:$SECRET:$PASSWORD:xxx *" >> /etc/ppp/otp-secrets

	echo ""
	echo "The configuration file has been written to $homeDir/$CLIENT.ovpn."
	echo "Download the .ovpn file and import it in your OpenVPN client."
	echo "OpenVPN username: $CLIENT"
	echo "OpenVPN password: $PASSWORD"
	echo "OpenVPN TOTP secret: $SECRET"
	echo "OpenVPN TOTP qrcode path: $homeDir/$CLIENT.png"
...
function revokeClient() {
	...
	find /home/ -maxdepth 2 -name "$CLIENT.png" -delete
	rm -f "/root/$CLIENT.ovpn"
	rm -f "/root/$CLIENT.png"
	sed -i "/^$CLIENT,.*/d" /etc/openvpn/ipp.txt
	sed -i "/CN=$CLIENT$/d" /etc/openvpn/easy-rsa/pki/index.txt
	cp /etc/openvpn/easy-rsa/pki/index.txt{,.bk}
	rm -f "/etc/openvpn/google-auth/$CLIENT"
	sed -i "/^$CLIENT otp.*/d" /etc/ppp/otp-secrets
